...
`).forEach(r=>{i.push(r)})}function b(){if(document.querySelector("#create-table"))return;const e=document.createElement("button");e.textContent="Create Table",e.addEventListener("click",()=>{a()}),e.id="create-table",document.querySelector("#app").appendChild(e)}async function x(){}const d=`■ 00:00-01:00  1, 4 та 5 черги
...
■ 23:00-24:00  4, 5 та 6 черги`,C="13.07.2024 at 18:50";document.getElementById("fileInput").addEventListener("input",function(e){const t=e.target.files[0];t&&y(t)});document.getElementById("submit-text-schedule").addEventListener("click",()=>{const e=document.getElementById("input-text-schedule");u(e.value)});document.getElementById("predefined-last-update").textContent="Last update: "+C;document.getElementById("load-predefined").addEventListener("click",()=>{console.log(d),u(d)});document.getElementById("load-from-tg-channel").addEventListener("click",x);m(document.getElementById("blackouts-order-table"));
